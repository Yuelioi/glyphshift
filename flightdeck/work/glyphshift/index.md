...
四个独立 Adapter，中文 presentation 在 Windows PowerShell 5.1 下按 UTF-8 稳定读取。桌面新增
“探针”产品页：用户显式选择一个软件和一组可观察 Adapter，目标 Runtime 有界采集原文，停止后
分别落盘带 Adapter provenance/次数/时间的 Capture Catalog 与纯原文字典草稿。
...
- 用真实授权软件运行探针，根据 Capture Catalog 的 Adapter provenance 和缺口决定是否增加
  DirectWrite、UI Automation 或框架专用 seam；没有证据时不扩展。
- 继续 Dictionary Catalog 内容寻址安装与恢复 Slice。
