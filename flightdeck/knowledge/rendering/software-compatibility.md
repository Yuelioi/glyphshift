...
| [Wireshark 4.6.8](../../work/software-text-coverage/references/software/wireshark.md) | **已验证可用（实机 Runtime）** | **Qt Translation Service（Qt 6.10.3 x64）** | 动态 MSVC Qt 6.10.3 的 `QCoreApplication::translate` 已完成 622 条 V2 observation、context 独立命中、真实 plural `n` observe-only、placeholder fail-open、三代标题刷新与停用恢复；默认生产 DLL 只接受 Qt 6.10.3，Qt 5.15.18 仅在显式研究 feature 下保留且不进入生产 Bundle。 |
...
| [Vovious](../../work/software-text-coverage/references/software/vovious.md) | **尚未适配；JUCE 6.1.6 canonical profile No-Go** | **等待新的 JUCE 通用完整字符串 seam / 其他精确版本 profile** | 静态 JUCE 6.1.6，现有通用文字 Adapter 0 命中。静态 locator 唯一恢复 `addCurtailedLineOfText` 与 `addFittedText`，但 canonical `addJustifiedText` 与 `addLineOfText` callable 入口缺失；按 fail-closed 门槛停止，没有进入实机 Hook，也不做软件专属地址补丁。 |
