...
- [通用实时翻译覆盖](work/real-time-translation-coverage/index.md)
...
- 新增通用 Adapter 的立项必须先绑定至少一个**相对知名、公开可获得、仍有现实用户价值的真实软件**作为代表目标，并证明该软件确实使用候选技术路径且存在现有 Adapter 无法覆盖的文字缺口。只有小众样本、框架 Demo、合成宿主或“理论上很多软件会用”时，只能做技术研究，不能据此新增生产 Adapter。代表目标只是验收锚点，正式实现仍必须按框架 / 渲染接口 / 稳定文字入口泛化，禁止写软件专属分支。
