...
fn native_package() -> PathBuf {
    ...
    profile.join("glyphshift_adapter_raylib_native.dll")
}
...
#[test]
fn raylib_activation_rejects_a_process_without_the_dynamic_module() {
    let package = unsafe {
        LoadedNativeAdapter::load(&native_package(), &glyphshift_adapter_raylib::descriptor())
            .expect("load verified raylib package")
    };
