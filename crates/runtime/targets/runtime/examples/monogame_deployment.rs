...
fn main() {
    ...
    let binding = AdapterBinding {
        ...
        host: AdapterHostBinding::TargetProcess {
            library: PackageArtifactId::new("adapters/monogame"),
        },
    ...
    for (generation, translation) in [
        ...
        let mut snapshot = TranslationSnapshot::empty(Generation::new(generation)).with_entry(
            "text",
            "AAA",
            translation,
        );
        snapshot = snapshot
            .with_entry("text", "A B A B", "中文中文")
            .with_entry("text", "B \r\nA B", "中")
            .with_entry("text", "B \r\nB A", "中")
            .with_entry("text", "B B \r\nA", "文");
        if generation == 7 {
            snapshot = snapshot.with_entry("text", " AAA ", "B");
        }
        let publication =
            RuntimePublication::new(RouteProgram::direct("text"), snapshot, FontPolicy::empty());
        fs::write(
            output.join(format!("publication-{generation}.json")),
            publication.encode_json().unwrap(),
        )
        .unwrap();
        ...
        .with_observation_producer(
            CaptureProducerConfiguration::new(
                CaptureProducerId::new("monogame-runtime-contract").unwrap(),
                generation,
            )
            .unwrap(),
        );
        fs::write(
            output.join(format!("deployment-{generation}.json")),
            deployment.encode_json().unwrap(),
        )
        .unwrap();
    }
}
