...
foreach ($variant in @(@('classic', '/DCLASSIC'), @('classic-layout', '/DCLASSIC /DOBJECT_PADDING=23'), @('classic-wrong-buffer', '/DCLASSIC /DWRONG_MEMBER'), @('classic-wrong-display', '/DCLASSIC /DWRONG_DISPLAY'), @('standard', ''), @('different-layout', '/DOBJECT_PADDING=17'), @('unrelated', '/DWRONG_TYPE'), @('wrong-buffer', '/DWRONG_MEMBER'), @('wrong-display', '/DWRONG_DISPLAY'))) {
