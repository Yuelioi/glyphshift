...
- **取词翻译：** 标题栏提供一个紧凑文字动作，不增加一级导航。Modal 先选择已登记软件和本地词典，
  再进入“指向文字并按 `Ctrl+Shift+F9`”等待态；读取中、原文/译文、未命中、错误、复制、重试和
  关闭都留在同一张有界结果表面。普通界面不展示 PID、target token、worker 或程序路径。
...
- 取词翻译使用 `Ctrl+Shift+F9` 完成一次 Point 请求：用户先在 Glyphshift 明确选择软件和词典，
  再切换到该软件并把鼠标指向文字；快捷键只读取一次，不注册全局鼠标点击 Hook。前台程序与所选
  软件不一致、权限不匹配、无文本、超时和取消均显示可恢复错误。Dictionary 未命中保留原文并
  说明当前没有生产在线 Provider；UIA 无结构化文本时后续只提供显式“尝试 OCR”，不静默截图。
