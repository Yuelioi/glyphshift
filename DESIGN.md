...
启用工作流就是持续期望，没有第二个“开始翻译”动作；标题栏的一次性取词是独立的显式读取任务，
不改变 Workflow actual state，也不冒充原位写回。
...
- **探针：** 管理可恢复的观察任务。每个任务绑定一个软件、一个词典和一组 Adapter；选择器按
  ...
  联合表展示原文、译文、状态与技术证据，译文编辑直接修改绑定词典。列表页主动作是“快速测试”：
  默认只选择或捕获正在运行的程序，检测通过后由 Desktop 一次建立临时软件、空白词典、兼容
  Adapter Plan 和运行中的 Probe；原手工“新建探针任务”作为次要高级路径保留。
...
- **设置：** 只在标题栏提供图标入口，不占主导航文字位置。设置页不提供在线翻译器或服务地址，
  展示真实生效的界面语言、主题、软件快速捕获/取词翻译快捷键、开机启动、关闭行为与当前权限；各项
  立即应用并持久化。快捷键通过单击控件后捕获真实按键组合来修改，不提供文本输入；系统占用或两个
  Glyphshift 动作相互冲突时留在录制态并保留旧值。管理员
  启动使用默认关闭的持久开关；开启后立即请求 UAC 重启，之后每次启动继续请求，关闭后从下一次
  启动恢复普通权限。当前权限是运行时事实，不能用开关状态代替。
- **取词翻译：** 标题栏提供一个紧凑文字动作，不增加一级导航。Modal 先选择已登记软件和本地词典，
  再进入“指向文字并按已配置快捷键（默认 `Ctrl+Shift+F9`）”等待态；读取中、原文/译文、未命中、错误、复制、重试和
  关闭都留在同一张有界结果表面。成功结果同时出现在光标附近的独立悬浮气泡：默认聚焦最近文字块，
  提供前后浏览、复制与关闭，主 Modal 仍保留完整结果。普通界面不展示 PID、target token、worker
  或程序路径。
...
进入录制，Esc 取消，不打开 Modal。Settings 作为标题栏进入的工具型子页，复用
...
- 标题、品牌、主导航、一次性取词动作、主题切换、帮助、设置与窗口控件位于同一条 48px 顶栏；
  不显示常驻桌面服务连接状态。主题和取词按钮具有随语言变化的可访问名称。
...
- 新增软件必须先通过基础接入检查；浏览程序后自动检查，也可手动重试。检查只确认正在运行、
  ...
  覆盖，不把预检通过表述为“已支持”。
- 软件快速捕获使用设备设置中的组合键（默认 `Ctrl+Shift+F8`）两段式状态：第一次进入等待，第二次读取前台进程并返回已填充
  的新增 Modal；捕获结果不自动保存。等待状态必须可取消，失败后保留等待并给出可恢复原因。
- 取词翻译使用设备设置中的组合键（默认 `Ctrl+Shift+F9`）完成一次 Point 请求：用户先在 Glyphshift 明确选择软件和词典，
  再切换到该软件并把鼠标指向文字；快捷键只读取一次，不注册全局鼠标点击 Hook。前台程序与所选
  软件不一致、权限不匹配、无文本、超时和取消均显示可恢复错误。Dictionary 未命中保留原文并
  说明当前没有生产在线 Provider；UIA 无结构化文本时只提供显式“尝试 OCR”，不静默截图。成功后
  无装饰气泡以当前光标所在显示器的可用边界定位，优先出现在光标右下方，越界时翻转并留出安全边距；
  显示气泡不主动聚焦主窗口，Esc 或关闭按钮只关闭当前气泡。气泡采用与主应用一致的石墨表面和钴蓝
  来源标识，不增加卡片嵌套，也不持久化结果。
- 修改软件快速捕获或取词快捷键时只接受一次真实 `keydown`，组合必须包含 Ctrl、Alt 或 Win 之一；
  候选先做不改变当前注册的可用性探测。保存时先注册候选再释放旧键，持久化失败则回绑旧键；系统
  占用、两个 Glyphshift 动作互相冲突、无效输入和进行中的对应请求都显示内联可恢复状态，旧快捷键
  继续有效。
...
- Quick Probe 详情以“保留”和“结束并清理”替代普通设置/删除动作；列表和恢复后的详情都显示临时
  标记。清理确认必须说明 ownership 边界，并在结束后明确反馈复用或被引用而保留的资产。
