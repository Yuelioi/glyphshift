...
- **设置：** 只在标题栏提供图标入口，不占主导航文字位置。设置页不提供在线翻译器；统一展示 AI
  单批条目数与输入 Token 预算，并管理可复用的 AI Profile。供应商服务地址只在 Profile 编辑器中出现，
  批次策略不随 Profile 切换。设置还展示真实生效的界面语言、主题、软件快速捕获快捷键、开机启动、
...
Settings 外观、全局 AI 批次策略、Workflow 基础配置与 Software 编辑属于同一类轻量配置表面，共享分组标题、说明与
