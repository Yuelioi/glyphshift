...
创建探针时默认创建普通字典，也可使用已有字典。成功创建后，停止或删除探针都保留软件和字典。
_Avoid_: 独立探针草稿
...
**排除字典（Excluded Dictionary）**：
探针可选择多个字典，排除其中已有译文的原文；过滤发生在分页、AI 翻译与导出之前，不修改排除字典，也不将其作为当前探针的替换规则。
_Avoid_: 临时资产、隐式合并字典

探针导入导出使用 JSON 或 CSV。导入默认覆盖相同原文，也可保留已有条目或替换当前字典全部条目；原始观测证据保持独立。
