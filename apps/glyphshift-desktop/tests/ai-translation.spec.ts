...
test('settings keeps confirmation global while batch size belongs to each AI profile', async ({ page }) => {
  ...
  await expect(aiSection.getByRole('heading', { name: '连接与模型' })).toBeVisible()
  await expect(aiSection.getByRole('button', { name: '添加 AI Profile' })).toBeVisible()
...
test('background translation task reports partial batches, writeback, usage, and model history', async ({ page }) => {
  ...
  await expect(statistics.getByRole('button', { name: '筛选模型协议' })).toBeVisible()
  ...
  await statistics.getByRole('textbox', { name: '搜索模型、Profile 或协议' }).fill('missing-model')
  await expect(statistics.getByText('没有符合条件的模型统计')).toBeVisible()
  await statistics.getByRole('textbox', { name: '搜索模型、Profile 或协议' }).fill('qwen3')
  await statistics.getByRole('button', { name: '显示列' }).click()
  await page.getByRole('menuitemcheckbox', { name: '协议', exact: true }).click()
  await page.keyboard.press('Escape')
  await expect(statistics.getByRole('columnheader', { name: '协议', exact: true })).toHaveCount(0)
