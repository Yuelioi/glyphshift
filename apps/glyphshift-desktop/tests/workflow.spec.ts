...
test('running workflow opens a bounded local decision diagnostics table', async ({ page }) => {
  ...
  await expect(dialog.getByText(/查看最近匹配到的原文和词典/)).toBeVisible()
...
test('workflow adapter catalog is one table with classification columns and select all', async ({ page }) => {
  ...
  await expect(adapterTable.getByRole('columnheader', { name: '兼容方式', exact: true })).toBeVisible()
  ...
  const adapterCheckboxes = adapterTable.getByRole('checkbox', { name: /^选择兼容方式 / })
  const checkedAdapterCheckboxes = adapterTable.getByRole('checkbox', { name: /^选择兼容方式 /, checked: true })
...
test('workflow saves font coverage and ordered inline candidates', async ({ page }) => {
  ...
  await expect(dialog.getByRole('tab', { name: '字体策略' })).toHaveAttribute('aria-selected', 'true')
  await expect(dialog.getByRole('combobox', { name: '应用范围' })).toContainText('探针找到的全部文字')
...
test('saving fonts keeps the editor section and target and allows another save', async ({ page }) => {
  await replaceModel(page, expandedModel())
  await page.getByRole('button', { name: '编辑 默认创作工作流' }).click()
  const editor = workflowEditor(page)
  await editor.getByRole('tab', { name: '字体策略' }).click()
  await editor.getByRole('button', { name: '当前软件目标' }).click()
  await page.getByRole('option', { name: /^Pixel Studio/ }).click()
  await editor.getByRole('switch', { name: '启用字体策略' }).click()
  ...
  await expect(editor.getByRole('button', { name: '当前软件目标' })).toContainText('Pixel Studio')
  ...
  await editor.getByRole('checkbox', { name: '选择字体 Synthetic Sans' }).check()
  await editor.getByRole('button', { name: '保存工作流' }).click()
