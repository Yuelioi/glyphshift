...
const model = {
  ...
  adapters: [
    { id: 'synthetic.ext-text-out', name: 'ExtTextOutW', summary: '拦截 GDI 高级文本输出；覆盖字距数组、裁剪选项和部分字形索引绘制', version: '1.0.0', platforms: ['windows'], technologies: ['GDI'], features: ['textObserve', 'textReplace', 'fontSubstitute'], technicalTarget: 'gdi32.dll!ExtTextOutW', documentationUrl: 'https://learn.microsoft.com/en-us/windows/win32/api/wingdi/nf-wingdi-exttextoutw', configuration: 'none' },
...
test('capture composable workflow and asset surfaces', async ({ page }) => {
  ...
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/workflows-composable.png' })
...
test('capture distilled dictionary editor, metadata, and guarded inline draft', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: '新建字典' })).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-create-distilled.png' })
  await page.getByRole('dialog', { name: '新建字典' }).getByRole('button', { name: '取消' }).click()
  ...
  await page.getByRole('dialog', { name: '字典设置' }).getByRole('button', { name: '取消' }).click()
  ...
  await page.setViewportSize({ width: 960, height: 640 })
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-editor-distilled-compact.png' })
  ...
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-inline-draft-compact.png' })
  await page.getByRole('button', { name: '返回字典列表' }).click()
  await expect(page.getByRole('dialog', { name: '放弃未保存更改？' })).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-discard-guard-compact.png' })
})
...
test('capture local dictionary provenance and offline catalog modes', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '字典', exact: true }).click()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-library-local.png' })

  await page.getByRole('button', { name: '在线目录', exact: true }).click()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-library-catalog-offline.png' })

  await page.setViewportSize({ width: 960, height: 640 })
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/dictionary-library-catalog-offline-compact.png' })
})
...
test('capture help and settings surfaces', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '完成第一次界面翻译' })).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/help-guide.png' })
  await page.getByRole('tab', { name: 'AI 翻译' }).click()
  await expect(page.getByRole('heading', { name: '用 AI 补全空白译文' })).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/help-ai-translation.png' })
  ...
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/settings-local-assets.png' })
  await page.setViewportSize({ width: 960, height: 640 })
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/settings-local-assets-compact.png' })
})
...
test('capture compact help surface', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: '完成第一次界面翻译' })).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/help-guide-compact.png' })
  await page.getByRole('tab', { name: '技术与兼容' }).click()
  await expect(page.getByRole('heading', { name: '当前适配器' })).toBeVisible()
  await waitForVisualStability(page)
...
test('capture English light settings at wide and compact widths', async ({ page }) => {
  ...
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/settings-english-light.png' })
...
test('capture light workflow and software editor surfaces', async ({ page }) => {
  ...
  await expect(page.getByTestId('workflow-editor')).toBeVisible()
  await waitForVisualStability(page)
  await page.screenshot({ path: '../../local-test/evidence/desktop-screens/workflow-editor-light.png' })
  await page.setViewportSize({ width: 960, height: 640 })
  await waitForVisualStability(page)
