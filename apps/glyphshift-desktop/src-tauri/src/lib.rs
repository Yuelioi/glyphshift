...
enum SoftwareQuickCaptureTransition {
    Armed,
    Capture,
}

#[derive(Debug, Default)]
struct SoftwareQuickCaptureState {
    armed: bool,
    shortcut_available: bool,
}

impl SoftwareQuickCaptureState {
    fn press(&mut self) -> SoftwareQuickCaptureTransition {
        if self.armed {
            self.armed = false;
    ...
    fn arm(&mut self) -> Result<(), CommandError> {
...
impl DesktopApplication {
    ...
    fn software_preflight_for(&self, executable: WindowsExecutable) -> SoftwarePreflightView {
...
#[tauri::command]
fn desktop_arm_software_capture(
...
#[tauri::command]
fn desktop_cancel_software_capture(
    capture: State<'_, Mutex<SoftwareQuickCaptureState>>,
) -> Result<(), CommandError> {
    capture
        .lock()
...
#[cfg(windows)]
fn handle_software_quick_capture_shortcut(app: &tauri::AppHandle) {
    let transition = app
...
pub fn run() {
        ...
        tauri_plugin_global_shortcut::Builder::new()
            .with_handler(|app, _shortcut, event| {
                if event.state() == tauri_plugin_global_shortcut::ShortcutState::Pressed {
                    handle_software_quick_capture_shortcut(app);
                }
            })
            .build(),
    );
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            desktop_preflight_software,
            desktop_arm_software_capture,
            desktop_cancel_software_capture,
