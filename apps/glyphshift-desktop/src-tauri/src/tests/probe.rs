...
#[test]
fn probe_plan_places_replacement_sources_before_observe_only_fallbacks() {
    let (mut application, _calls, software_id, _data_root) = workflow_application();
    application.adapters = vec![
        AdapterView {
            id: "test.observe-only".into(),
    ...
    let _ = software_id;
    assert_eq!(
        application.prioritize_probe_adapter_ids(&[
            Box::<str>::from("test.observe-only"),
            Box::<str>::from(TEST_ADAPTER_ID),
        ]),
        vec![
            Box::<str>::from(TEST_ADAPTER_ID),
            Box::<str>::from("test.observe-only"),
